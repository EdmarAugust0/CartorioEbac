...
int registro(){
	...
	file = fopen(arquivo, "w"); // cria o arquivo // "w" pois estamos criando um novo
	...
	file = fopen(arquivo, "a"); // "a" pois estamos atualizando 
	fprintf(file,nome);
	fclose(file);
...
int main(){
	...
	for(x = 1; x = 1;){
		...
		scanf("%d", &opcao); // amarzenando ecolha do usuário
		
		system("cls"); // isso limpa os comandos dps de esolher a opção
